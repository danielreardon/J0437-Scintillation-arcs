...
for iarc in range(14):
    ...
    priors['vism_ra'] = bilby.core.prior.Uniform(-1000, 1000, 'vism_ra')  # km/s
    priors['vism_dec'] = bilby.core.prior.Uniform(-1000, 1000, 'vism_dec')  # km/s
    ...
    priors['vism_zeta'] = bilby.core.prior.Uniform(-1000, 1000, 'vism_zeta')  # km/s
    priors['KOM'] = bilby.core.prior.Normal(207, 2.4, 'KOM')  # degrees
    priors['F'] = bilby.core.prior.Uniform(0, 100, 'F')
    priors['Q'] = bilby.core.prior.Uniform(0, np.std(veff), 'Q')  # km/s
    ...
    for i in range(100):
        ...
        vism_zeta_i = np.random.normal(loc=vism_zeta, scale=vism_zeta_err, size=1)
        ...
        pmra, pmdec = make_lsr(D, pars["RAJ"], pars["DECJ"], pm_ra_cosdec, pm_dec, vr=0)
        pmra_v.append(pmra * masrad * D * kmpkpc / secperyr)
        pmdec_v.append(pmdec * masrad * D * kmpkpc / secperyr)

        pmra_a, pmdec_a = make_lsr(D_a, pars["RAJ"], pars["DECJ"], pm_ra_cosdec_a, pm_dec_a, vr=0)
    ...
    print("VISM ZETA ", vism_zeta, np.mean(pmzeta_v), np.mean(pmzeta_v) - vism_zeta)

    print("TOTAL", np.sqrt(vism_ra**2 + vism_dec**2), np.sqrt(np.mean(pmra_v)**2 + np.mean(pmdec_v)**2))
    print(" ")
    print(" ")
    ...
    arcstring_new = "{} & {} & {} & {} & {} & {} & {} & {} \\\\ \n".format(label,
        make_string(s_i, s_i_err),
        make_string(np.mean(pmra_v), np.std(pmra_v)),
        make_string(np.mean(pmdec_v), np.std(pmdec_v)),
        make_string(s, s_err),
        make_string(zeta, zeta_err),
        make_string(np.mean(pmzeta_v), np.std(pmzeta_v)),
        round_sig(lnbf,2))
...
for i in range(3):
    ...
    np.savez(datadir + '/arcmodel{}.npz'.format(14 + i), s=ss[i], zeta=zetaarr[i], vism_zeta=vzeta[i])
...
lsrcoord = make_lsr_new(0.15679, pars["RAJ"], pars["DECJ"], pars['PMRA'], pars['PMDEC'], vr=-60.1)
